...
**Fünf Checks dieser Tabelle sind `enforced`, nicht einer:** `FID-001`, `FID-002`, `FID-003` und `FID-005` führen kein `adoption`-Feld, und ohne das Feld gilt `enforced`. Dazu `ARCH-003`. Ein Verstoss gegen einen der fünf blockiert. Was `ARCH-003` allein auszeichnet, ist die **Anwendbarkeit**: Es gilt `always`, die vier `FID`-Checks nur bei `tools_make_external_requests == true` — für einen datenabfragenden Server also ebenfalls immer. `advisory` sind `FID-006`, `FID-007`, `OPS-009`, `ARCH-018`, `ARCH-020` und `HITL-006`:
