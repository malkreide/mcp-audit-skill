...
Stand des Katalogs: `mcp-audit` auf `main` — 120 Checks in zwölf Kategorien auf zwei Spec-Baselines, davon sieben in der Kategorie `FID`. Geschnitten ist v2.2.0 (116 Checks); die vier Änderungen darüber hinaus stehen drüben unter `[Unreleased]`, darunter die beiden, die diese Tabelle zuletzt bewegt haben. Die Zuordnung ist nicht eins zu eins — `FID-003` trägt vier Regeln, `ARCH-020` zwei, Regel 5 braucht drei Checks und Regel 9 drei. Ohne Check ist nur noch Regel 12, und Regel 10 bringt mit `ARCH-003` den einzigen **`enforced`** Check dieser Tabelle mit — alle anderen sind `advisory`:
...
| 13 — Der Feldname samt Schreibweise | [`DRIFT-007`](https://github.com/malkreide/mcp-audit-skill/blob/main/checks/DRIFT-007.md) — «Feldnamen sind Teil des Vertrags», `severity: high`, `adoption: advisory`, derselbe Belegfall wie hier. Der Check hat §2.5 mit derselben Begründung durchlaufen, die oben in der Abgrenzung steht: `FID-006` **fängt** den Fall, aber die Behebung ist eine andere — strenge Bestätigung ergäbe «sechs korrekte Prüfungen, die beim nächsten Wechsel genauso laut scheitern wie vorher leise». Eigene Dimension drüben: Unempfindlichkeit gegen die Vertragsänderung statt ihrer Entdeckung. **Reichweite, zwei Stellen.** Erstens sind seine Pass-Criteria ein **`oder`**: normalisieren *oder* je Endpunkt ein Test gegen die echte Antwort. Wer den zweiten Arm nimmt, besteht drüben — und steht auf einer Quelle, die pro Endpunkt anders schreibt, genau in dem Zustand, gegen den die Description des Checks selbst argumentiert. Regel 13 kennt diesen Arm nicht: Sie verlangt die Normalisierung und stellt die Bestätigung **dahinter**, nicht daneben. Zweitens verlangt kein Kriterium die Kollisionsprüfung — dass zwei Schlüssel, die unter der Normalisierung zusammenfallen, in den Fehlerkanal gehören, steht allein hier |
