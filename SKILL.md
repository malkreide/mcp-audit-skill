...
- [ ] Jede Fixture nennt ihre Herkunft **und** ihr Aufzeichnungsdatum — ohne Datum ist «aufgezeichnet» von «ausgedacht» nicht zu unterscheiden (Regel 5)
...
Stand des Katalogs: `mcp-audit` auf `main` — 120 Checks in zwölf Kategorien auf zwei Spec-Baselines, davon sieben in der Kategorie `FID`. Geschnitten ist **v2.3.0** vom 2026-08-08; dieser Stand ist damit released und nicht mehr unterwegs. `DRIFT` hat ebenfalls sieben Checks, nummeriert `001`–`006` und `008`: **Die 007 ist verbrannt** — der Check dieser Nummer wurde am 2026-08-07 zurückgezogen und ging in `FID-006` auf, und die Nummer wird bewusst nicht neu vergeben, damit der Audit-Trail eindeutig bleibt. Was das für Zeile 13 heisst, steht dort. Die Zuordnung ist nicht eins zu eins — `FID-003` trägt vier Regeln, `FID-006` zwei, `ARCH-020` zwei, Regel 5 braucht drei Checks und Regel 9 drei. Ohne Check ist nur noch Regel 12.

**Sieben Checks dieser Tabelle sind `enforced`, nicht einer:** `FID-001`, `FID-002`, `FID-003`, `FID-004`, `FID-005` und `DRIFT-002` führen kein `adoption`-Feld, und ohne das Feld gilt `enforced`. Dazu `ARCH-003`. Ein Verstoss gegen einen der sieben blockiert. Was `ARCH-003` allein auszeichnet, ist die **Anwendbarkeit**: Es gilt `always`, die übrigen sechs nur bei `tools_make_external_requests == true` — für einen datenabfragenden Server also ebenfalls immer. `advisory` sind `FID-006`, `FID-007`, `OPS-009`, `ARCH-018`, `ARCH-020` und `HITL-006`:

| Regel | Check |
|---|---|
| 1 — Scope-Parameter explizit senden | [`FID-001`](https://github.com/malkreide/mcp-audit-skill/blob/main/checks/FID-001.md) — «Scope-Defaults: Filter-Parameter explizit senden, nie erben». **Reichweite:** Der Check verlangt, dass eine bewusst gewählte Einschränkung im Tool-Result **sichtbar** ist. Dass ihre **Begründung** den erreichbaren Scope zitiert — die Rubriken nennen, die das Risiko tragen, und sie in der Scope-Aufzählung nachweisen —, verlangt drüben keiner der 120 Checks; das steht allein hier |
...
| 5 — Syntax in der Description, Recall in den Tests | [`FID-005`](https://github.com/malkreide/mcp-audit-skill/blob/main/checks/FID-005.md) für die Syntax, [`FID-002`](https://github.com/malkreide/mcp-audit-skill/blob/main/checks/FID-002.md) für den Recall gegen die offizielle Oberfläche. **Reichweite:** Der dritte Teil der Regel — exakte Gleichheit statt Teilzeichenkette beim Vergleich auf ein strukturiertes Feld — wird von keinem der beiden gemessen. Beide lesen die Antwort des Servers; dass eine Zusicherung *über* sie schwächer formuliert ist, als ihr Name behauptet, steht in keinem Check-Kriterium. Der vierte Teil — die datierte Herkunft der Fixture — liegt dagegen auf [`OPS-009`](https://github.com/malkreide/mcp-audit-skill/blob/main/checks/OPS-009.md), und die Entstehung ist der Umweg wert: Der Check kam aus einer Katalog-Lücke **gegen diesen Skill** («vier Checks sagen ‹gegen die echte Antwort prüfen›, keiner sagt, woher die Antwort im Repo stammt»), und beim Nachlesen zeigte sich, dass die Lücke hier genauso offen war — der Skill nannte die Mock-Blindheit viermal und die Pflicht zur datierten Herkunft kein einziges Mal. Sie steht jetzt bei Regel 5, und der Check misst sie. **Reichweite:** `OPS-009` verlangt Herkunft und Datum; dass der Abruf **wiederholbar** danebenliegt, statt ein Handgriff im Gedächtnis zu sein, steht allein hier |
...
| 13 — Der Feldname samt Schreibweise | [`FID-006`](https://github.com/malkreide/mcp-audit-skill/blob/main/checks/FID-006.md) — «Antwortstruktur und Feldnamen bestätigen, bevor gezählt wird», `severity: high`, `adoption: advisory`, derselbe Belegfall wie hier. **Diese Zeile nannte bis zuletzt `DRIFT-007`, und den Check gibt es nicht mehr.** Er stand vier Tage im Katalog, war nie in einem Release und wurde am 2026-08-07 zurückgezogen: Beide prüfen dasselbe Objekt — die Namen, die der Code an der Antwort anfasst —, mit derselben Ursache und derselben Testgrenze, und ihre Pass-Kriterien mussten einander als Ausweg zitieren. Zwei Checks, die das tun, sind ein Check mit zwei Abschnitten. **Die Abgrenzung dieser Zeile ist damit hinfällig, und zwar zu Recht:** Sie argumentierte, drüben stehe ein `oder` — normalisieren *oder* je Endpunkt ein Test gegen die echte Antwort —, und Regel 13 kenne diesen zweiten Arm nicht. `FID-006` kennt ihn inzwischen ebenfalls nicht: Normalisieren an der Parse-Grenze ist dort ein **eigenes** Pass-Kriterium, der Test gegen die echte Antwort steht **zusätzlich** daneben, und ein festverdrahtetes Literal je Endpunkt ist ausdrücklich ein Fail. Genau die Lücke, gegen die diese Regel geschrieben wurde, ist drüben geschlossen. **Reichweite, was bleibt:** Kein Kriterium verlangt die Kollisionsprüfung. `FID-006` verbietet, unter der Normalisierung zwei **verschiedene** Namen zusammenzuziehen (`anzahl_total` bleibt von `anzahlTotal` verschieden) — dass ein tatsächlich eingetretener Zusammenfall in den **Fehlerkanal** gehört und nicht in eine stille Auswahl, steht weiterhin allein hier |
| 14 — Eine Zahlenspalte ohne Zahlen | [`FID-007`](https://github.com/malkreide/mcp-audit-skill/blob/main/checks/FID-007.md) — «Eine Zahlenspalte ohne Zahlen», `severity: high`, `adoption: advisory`, ebenfalls aus demselben Belegfall und mit denselben gemessenen Anteilen. Die Rangfolge der drei Umgänge steht dort ausgeschrieben, samt der Einstufung der stillen `0` als **schlimmer** als der Absturz, und die Kennzeichnung ist ausdrücklich der Gegenstand des Checks und nicht das Ausnehmen. **Reichweite:** Er geht in zwei Punkten **weiter** als diese Regel, und beide gehören beim Bauen mitgelesen — der Hinweis muss die **Richtung** der Abweichung nennen («die echten Werte liegen höher»), und ein Live-Test muss die tatsächlich vorkommenden Marker gegen die eigene Liste halten, weil nur die Quelle weiss, welche sie benutzt. Was drüben dagegen fehlt: `FID-007` misst die Konvertierung und das Tool-Result, nicht die **abgeleiteten** Grössen daneben — eine Quote oder eine Rangfolge, die dieselben Zeilen stillschweigend auslässt, fällt unter keines seiner Kriterien |

Wer nach den Regeln 1–5 baut, besteht die `FID`-Checks — und diese vier (`FID-001`, `FID-002`, `FID-003`, `FID-005`) sind `enforced`: Ein Verstoss blockiert, er wird nicht bloss gezählt. Regel 6 liegt seit `FID-006` ebenfalls dort, allerdings `advisory` — der Check verlässt diesen Stand erst nach dem ersten Portfolio-Durchlauf, der zeigt, wie viele Server die Antwortstruktur überhaupt bestätigen. Die Regeln 7–9 laufen über `ARCH-020`, `HITL-006`, `ARCH-018` und die `2026-07-28`-Kriterien von `FID-003` und sind `advisory` — sie blockieren also nicht. Regel 10 liegt auf `ARCH-003`, ebenfalls `enforced`, und als einziges dieser Tabelle `always` statt nur bei externen Anfragen. Regel 11 liegt zur Hälfte auf `FID-001` und `FID-003`. Regel 13 liegt seit der Rücknahme von `DRIFT-007` mit auf `FID-006`, Regel 14 auf `FID-007`, beide `advisory` und beide aus demselben Belegfall wie hier. Ohne Check ist damit nur noch Regel 12 — was je Zeile offen bleibt, steht dort unter «Reichweite», und bei Zeile 12 ist es ausdrücklich eine Lücke und kein Rand.

**Warum die Zeilen 7–9 nicht in `FID` liegen:** Die drei Regeln kommen aus der Spec, und der Katalog ordnet Spec-Checks nach dem Ort der Änderung ein — Caching und Reihenfolge nach `ARCH`, MRTR nach `HITL`. Die Datentreue-Hälfte davon stand hier bis zuletzt als offen und ist inzwischen abgedeckt — aber nicht durch neue `FID`-Checks: `ARCH-020` hat den Pagination-Schnitt und die `ttlMs`-Ableitung in sich aufgenommen, statt dafür einen eigenen `FID`-Check zu eröffnen, weil beides an denselben zwei Grössen hängt wie der bestehende Check; und die Disjunktheit gegen den Null-Treffer ist nach `FID-003` gegangen, weil sie an der Leermenge hängt und nicht am Rückfrageprotokoll.
...
**Also kein eigener `FID`-Check, sondern ein dritter Modus in `ARCH-003`** — derselbe Ausgang wie bei den Regeln 7 und 8, wo `ARCH-020` den Pagination-Schnitt und die `ttlMs`-Ableitung aufgenommen hat, statt einen Check zu eröffnen. Der Vorschlag liegt drüben als [`mcp-audit-skill#102`](https://github.com/malkreide/mcp-audit-skill/issues/102); bis er umgesetzt ist, steht der Rand in Zeile 10, wie überall sonst in dieser Tabelle.

**Zur Haltbarkeit dieser Tabelle:** Der Katalog bewegt sich schneller als dieser Skill. Zwischen v1.7.0 vom 2026-08-03 (97 Checks) und v2.0.0 vom 2026-08-04 (112, dual baseline) lag **ein** Tag, und in diesem einen Tag sind aus vier angeblich fehlenden Checks drei vorhandene geworden. Der nächste Beleg kam einen Tag später: aus «Regel 6 hat keinen Check» ein `FID-006` — plus zwei Reichweite-Sätze, die von «prüft er nicht» auf «prüft er» gekippt sind.

Der dritte Beleg ist der schärfste, weil er die Regeln traf, die **am selben Tag** hier dazukamen: Die Zeilen 13 und 14 standen keine 24 Stunden als «kein Check deckt sie ab», bevor `DRIFT-007` und `FID-007` drüben aufgingen — beide aus demselben Belegfall, beide mit derselben §2.5-Begründung, die hier in der Abgrenzung steht. Zwei Zeilen, die als offene Frage geschrieben waren, waren beim Schreiben schon beantwortet. Aufgefallen ist das nicht beim Lesen, sondern **Prüfung 14** im Wochenplan: 113 gegen 120 Checks, sechs gegen sieben `FID`. Genau dafür läuft sie an einem Zeitplan statt an einem Diff.

**Der vierte Beleg ist der unbequemste, weil er in die andere Richtung geht.** `DRIFT-007` — der Check, den der dritte Beleg als Antwort auf Zeile 13 feierte — ist am Tag darauf **zurückgezogen** worden und in `FID-006` aufgegangen. Die Zeile stand danach mit einem toten Link auf `checks/DRIFT-007.md` und mit einer Abgrenzung, die eine Lücke beschrieb, die es nicht mehr gab. Bis dahin war die Lehre dieses Abschnitts: *Eine Zeile veraltet, weil drüben ein Check dazukommt.* Sie ist jetzt die allgemeinere: **Eine Zuordnung veraltet in beide Richtungen — ein Check kann auch verschwinden, und dann wird aus einer belegten Abgrenzung eine erfundene.** Ein Zugewinn meldet sich immerhin als «nicht abgedeckt, obwohl abgedeckt»; eine Rücknahme meldet sich als gar nichts, bis jemand dem Link folgt.

Aufgefallen ist auch das nicht beim Lesen, sondern wieder an Prüfung 14 — und dort an dem einen Kriterium, das nicht zählt, sondern nachschlägt: *Die Tabelle verlinkt `DRIFT-007`, im Katalog gibt es sie nicht.* Die Zahlen waren zu diesem Zeitpunkt richtig (120 Checks, zwölf Kategorien, sieben `FID`). **Eine Prüfung, die nur Summen vergleicht, hätte hier bestanden**, weil `DRIFT` seine sieben behalten hat: Die 007 ging, die 008 kam. Deshalb prüft sie jede genannte ID einzeln gegen den Katalog und nicht die Zahl darüber.

**Und noch ein drittes, seit demselben Anlass:** Die Einstufung — `enforced` oder `advisory` — hält sie jetzt ebenfalls je Check gegen das Frontmatter drüben. Auch dafür ist der Beleg ein Fehler, den Zahlen und IDs beide durchgelassen haben: Hier stand «`ARCH-003` ist der einzige `enforced` Check dieser Tabelle», während sechs weitere es ebenfalls sind. Jede Summe stimmte, jede ID existierte, der Lauf war grün. **Eine Einstufung ist kein Zählwert** — sie sagt, ob ein Verstoss blockiert, und darauf stützt jemand seinen Build. Beim ersten Lauf dieser Erweiterung fielen prompt `FID-004` und `DRIFT-002` auf, die in der Aufzählung oben schlicht fehlten; die Prüfung verlangt seither, dass die beiden Listen die verlinkten Checks **genau aufteilen**, weil Nichterwähnung sich wie «nicht betroffen» liest.

Und ein Nebeneffekt, der zur Sache gehört: Der Satz «statt ein `FID-007` zu eröffnen» stand zweimal in diesem Abschnitt und meinte eine Nummer, die es absichtlich nicht gab. Seit es sie gibt, las er sich, als sei dieser Check gemeint — er nennt jetzt keine Nummer mehr. Der Kommentar in `tools/checks/catalogue.py`, der genau diesen Tag vorhergesagt hat, ist mitgezogen.

Wer hier eine Zeile liest, um ein Finding zu beheben, prüft besser den Katalogstand oben mit — eine falsche Zuordnung kostet an genau der Stelle am meisten, an der jemand etwas reparieren will.
