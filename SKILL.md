...
Stand des Katalogs: `mcp-audit` v2.0.0, 112 Checks in zwölf Kategorien auf zwei Spec-Baselines, davon fünf in der Kategorie `FID`. Die Zuordnung ist nicht eins zu eins — zwei Regeln teilen sich einen Check, eine Regel braucht zwei, zwei weitere teilen sich einen ausserhalb von `FID`, und eine hat keinen:
...
| 6 — Antwortstruktur bestätigen, bevor gezählt wird | **kein Check.** Ein `FID-006` existiert nicht; kein Check des Katalogs fragt, ob eine Strukturabweichung upstream im Fehlerkanal endet statt in einer leeren Liste. `DRIFT-002` («Fallback verengt, erweitert nie») liegt daneben, nicht darauf: dort wird ein *anderer* Datensatz geliefert, hier *keiner* |
| 7 — Deterministische Reihenfolge | [`ARCH-020`](https://github.com/malkreide/mcp-audit-skill/blob/main/checks/ARCH-020.md) — «`ttlMs` und `cacheScope` auf List- und Read-Ergebnissen, deterministische Reihenfolge», `spec_baseline: 2026-07-28`, `adoption: advisory`. **Reichweite:** Der Check prüft die Reihenfolge von `tools/list`, und zwar über Prozessgrenzen hinweg mit `PYTHONHASHSEED=random` — schärfer als das Testrezept oben. Den Pagination-Schnitt auf Query-Resultaten prüft er nicht |
| 8 — Ehrliches `ttlMs` | ebenfalls [`ARCH-020`](https://github.com/malkreide/mcp-audit-skill/blob/main/checks/ARCH-020.md) — der Check trägt beide Hälften, samt dem Kriterium «kein Wert oberhalb der Änderungsfrequenz der Quelle» und `cacheScope: "public"` nur über aufruferunabhängigen Inhalten. **Reichweite:** Er misst die fünf `CacheableResult`-Methoden und knüpft `cacheScope` an `data_class`; die Ableitung eines `ttlMs` für Datenresultate aus `source_freshness` fragt er nicht ab |
| 9 — `input_required` ist keine leere Antwort | [`HITL-006`](https://github.com/malkreide/mcp-audit-skill/blob/main/checks/HITL-006.md) — «MRTR statt serverinitiierter Requests», dazu [`ARCH-018`](https://github.com/malkreide/mcp-audit-skill/blob/main/checks/ARCH-018.md) für `resultType` auf allen Results. **Reichweite:** Beide prüfen die MRTR-Seite — Retry-Idempotenz, kein `input_required` für gewöhnliche Fehler. Die Abgrenzung gegen den Null-Treffer aus `FID-003` (kein `hint` auf einer Rückfrage, kein `inputRequests` auf einer Leermenge) steht in keinem der beiden |

Wer nach den Regeln 1–5 baut, besteht die `FID`-Checks; die Regeln 7–9 laufen über `ARCH-020`, `HITL-006` und `ARCH-018` und sind dort `advisory` — sie blockieren also nicht, sie werden gezählt. Ohne Check ist einzig Regel 6: Sie beschreibt einen Fehler, den der Katalog derzeit nicht sieht, und ein Audit ohne Befund ist dort kein Beleg.

**Warum die Zeilen 7–9 nicht in `FID` liegen:** Die drei Regeln kommen aus der Spec, und der Katalog ordnet Spec-Checks nach dem Ort der Änderung ein — Caching und Reihenfolge nach `ARCH`, MRTR nach `HITL`. Die Datentreue-Hälfte davon, die in `FID` gehörte, ist genau die Reichweite, die oben je Zeile als offen ausgewiesen ist.

**Zur Haltbarkeit dieser Tabelle:** Der Katalog bewegt sich schneller als dieser Skill. Zwischen v1.7.0 (97 Checks) und v2.0.0 (112, dual baseline) lagen vier Tage, und mit v2.0.0 sind aus vier angeblich fehlenden Checks drei vorhandene geworden. Wer hier eine Zeile liest, um ein Finding zu beheben, prüft besser den Katalogstand oben mit — eine falsche Zuordnung kostet an genau der Stelle am meisten, an der jemand etwas reparieren will.
