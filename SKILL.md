...
| 5 — Syntax in der Description, Recall in den Tests | [`FID-005`](https://github.com/malkreide/mcp-audit-skill/blob/main/checks/FID-005.md) für die Syntax, [`FID-002`](https://github.com/malkreide/mcp-audit-skill/blob/main/checks/FID-002.md) für den Recall gegen die offizielle Oberfläche. **Reichweite:** Der dritte Teil der Regel — exakte Gleichheit statt Teilzeichenkette beim Vergleich auf ein strukturiertes Feld — wird von keinem der beiden gemessen. Beide lesen die Antwort des Servers; dass eine Zusicherung *über* sie schwächer formuliert ist, als ihr Name behauptet, steht in keinem Check-Kriterium. Die andere Hälfte dieser Regel — *ein Mock bildet die Annahme ab, mit der er geschrieben wurde* — hat seit [`OPS-009`](https://github.com/malkreide/mcp-audit-skill/blob/main/checks/OPS-009.md) einen eigenen Check, und der ist ausdrücklich aus einer Katalog-Lücke **gegen diesen Skill** entstanden: Vier Checks sagen «gegen die echte Antwort prüfen», und keiner sagte, woher die Antwort im Repo stammt. `OPS-009` verlangt Herkunft **und Datum** der Fixture. Er ersetzt den Live-Canary nicht — er schliesst die Lücke daneben |
