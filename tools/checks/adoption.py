...
wurde rot, weil kein Schritt hinsah: Die Datei kompilierte (Check 2),
importierte (Check 3) und bestand beide Ruff-Gates. Alle vier haben recht — sie
...
MANIFEST = "reference/adoption.toml"
...
def _evaluate(prop: dict, scope: list[ast.AST], where: str) -> bool:
    ...
    raise CheckFailed(
        ...
        "  Entweder die Art in tools/checks/adoption.py ergänzen, oder sie im "
...
@register(17, "the templates hold what the adoption manifest claims")
def templates_hold_their_properties(root: Path) -> str:
    ...
    for template in templates:
        ...
        try:
            ...
            # kaputt, und Check 2 sagt es genauer.
            raise CheckFailed(
                f"{rel}: Syntaxfehler in Zeile {exc.lineno} — {exc.msg}.\n"
                "  Ohne lesbaren Quelltext lässt sich keine Eigenschaft "
                "messen. Check 2 meldet dasselbe und ist die Stelle, an der "
                "das zu reparieren ist."
            ) from exc
    ...
    unmapped = sorted(
        f"reference/{p.name}"
        for p in sorted((root / "reference").glob("*.py"))
