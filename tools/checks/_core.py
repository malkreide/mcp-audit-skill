...
Dass das hier kein theoretisches Risiko ist, steht in `ruff.toml`: Dort stand
`select = []`, beide Ruff-Schritte meldeten «All checks passed!», und niemand
merkte es, weil nichts rot wurde. Der Katalog-Job liefert den zweiten Beleg
aus der Gegenrichtung — er schlug bei seinem ersten Lauf falsch an, und auch
das war ungetestet.
...
Lesenden zur falschen Datei. Regel 5 dieses Skills sagt, was danach kommt:
Ein Test, der ständig falsch anschlägt, wird abgeschaltet und fängt dann gar
nichts mehr.

Das Gerüst ist dasselbe wie in `mcp-data-source-probe-skill`. Zwei Kopien
statt eines geteilten Pakets, mit Absicht: Beide Repositories sind
Dokumentation plus Vorlagen, keines installiert etwas, und eine gemeinsame
Abhängigkeit zwischen ihnen wäre mehr Maschinerie als die Sache trägt.
"""
...
def register(
    ...
    *,
    ...
    auf. Sie doppelt zu vergeben ist ein Fehler beim Import, nicht erst zur
    Laufzeit — sonst verdeckt die zweite Prüfung stillschweigend die erste.
    """
    ...
    try:
        ...
        return Result(
            ...
            output=(
                ...
                "tools/checks, kein Befund über das Repository.\n"
