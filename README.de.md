...
Output: Profil-getriebene Auswahl der ~30 anwendbaren Checks aus 120, automatisierte Verifikation aller `automated`/`config_check`/`documentation_check`-Modi, Findings-Stubs für `code_review`/`runtime_test`-Modi, vollständiger Audit-Report nach Template — alles in `<repo>/audits/YYYY-MM-DD-<server-name>/`.

## Standards-Provenance

Die 120 Checks stammen aus zwei kuratierten Best-Practice-Dokumenten plus fünf eigenen Layern (Schweiz-Compliance, Datentreue, Identität, Upstream-Drift, Abhängigkeitsauflösung) in auditierbarer Form. Jeder Check trägt im Frontmatter eine `pdf_ref`-Referenz auf seine Quelle.
...
| **Datentreue-Layer** | Scope-Defaults, Recall gegen Ground Truth, Leermenge ≠ Abwesenheit, Query-Syntax, Antwortstruktur bestätigen bevor gezählt wird, unterdrückte Zahlwerte. Abgeleitet aus einem realen Portfolio-Vorfall ([termdat-mcp#11](https://github.com/malkreide/termdat-mcp/issues/11)) und aus einer Registry-Abfrage, die eine Ebene über den Feldern las | 7 Checks (`FID-*`) |
...
Der Skill bringt einen Slash-Command mit, der den 8-Schritte-Workflow als Claude-Code-Workflow ausführt — Profil-Load, Applicability-Filter, automatisierte Check-Ausführung, Findings-Generierung und Report-Erstellung in einem Lauf.
...
| `OPS` | Test-Strategie, Doku-Standard, Phasenarchitektur, Audit-Redlichkeit, Pipeline-Ehrlichkeit, reproduzierbare Urteile, ausführbare Anleitungen, prüfbare Guards, Herkunft der Fixtures, Gegenprobe als Abnahmekriterium | Anhang C + Custom | 10 | — · 7 high · 3 medium |
...
| `DRIFT` | Upstream-Vertrag und Repo-Prosa: Endpoint-Drift, Fallback-Semantik, Testgüte, CHANGELOG gegen Code | Custom | 7 | — · 4 high · 3 medium |
...
Das Feld ist optional; fehlt es, gilt `enforced`. Von 120 Checks sind genau fünfundzwanzig `advisory`: `ARCH-022`, `FID-006`, `FID-007`, `OBS-008`, `OPS-005`, `OPS-006`, `OPS-007`, `OPS-008`, `OPS-009`, `OPS-010`, `DRIFT-008` — dazu die vierzehn Migrations-Checks `ARCH-015`, `ARCH-016`, `ARCH-017`, `ARCH-018`, `ARCH-019`, `ARCH-020`, `ARCH-021`, `HITL-006`, `SCALE-008`, `SCALE-009`, `SCALE-010`, `SEC-025`, `SEC-026` und `SEC-027`. `DEP-001`, `DRIFT-006`, `OBS-007` und `ARCH-014` gingen denselben Weg und sind inzwischen auf `enforced` promoviert — die Brücke soll eine Handvoll neuer Checks tragen, nicht volllaufen.

**Fünfundzwanzig sind nicht fünfundzwanzig gewöhnliche Checks auf der Brücke.** Vierzehn davon sind die Migrations-Kohorte, die geschlossen abgeht; elf sind gewöhnlich, und vier Checks sind bereits auf `enforced` hinübergegangen. Das ist der Mechanismus bei der Arbeit, nicht die volllaufende Brücke — und der Wächter misst es genauso: Die Quote in `tests/test_adoption_stage.py` zählt den Rest ohne die Kohorte.
...
`FID-006` gehört nicht zu dieser Kohorte und geht nicht mit ihr ab. Er ist aus dem gewöhnlichen Grund `advisory`: Seine Fail-Patterns — `payload.get("servers", [])` und eine fest verdrahtete Schreibweise der Kopfzeile — sind das normale Idiom, `enforced` auf `high` am Merge-Tag hätte also fast jeden Datenquellen-Server rot gefärbt, für eine Eigenschaft, nach der nie jemand gefragt hat. Er verlässt `advisory` nach dem ersten Portfolio-Durchlauf, der zeigt, wie viele Server die Antwortstruktur bestätigen und wie oft das Normalisieren der Schreibweise die ehrliche Antwort ist statt einer Ausrede.
...
2. **Katalog laden** — alle 120 Checks parsen
...
├── tests/              eine Suite je Skill, dazu die Guards der Repo-Ebene
...
- ✅ Reference-Summary
- ✅ Check-Katalog: **120 Checks, alle 12 Kategorien vollständig**
