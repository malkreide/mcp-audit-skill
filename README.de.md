...
5. **Query-Syntax in die Description, Recall in die Tests.** Abfragesprache und Matching-Granularität dokumentieren; Recall über Live-Untergrenzen absichern, denn ein Mock bildet die Annahme ab, mit der er geschrieben wurde. Aus demselben Grund nennt jede Fixture ihre Herkunft **und ihr Datum**: Ohne Datum ist «aufgezeichnet» nach zwei Jahren von «ausgedacht» nicht mehr zu unterscheiden — der Canary misst die Quelle, die Herkunft datiert den Mock. Und auf strukturierten Feldern exakt vergleichen, nie auf Teilzeichenkette: Ein Präfix-Assert besteht, bis der Feldwert wächst — dann besteht er weiter und meint etwas anderes.
...
Wer nach den Regeln 1–6 baut, besteht die `FID`-Checks; wer sie beim Audit reisst, findet hier die Behebung. Die Regeln 7–9 sind ausserhalb von `FID` abgedeckt, und diese Checks sind `advisory` — sie werden gezählt, nicht erzwungen. Regel 10 ist die Ausnahme: `ARCH-003` blockiert. Ohne Check ist einzig Regel 12; bei den übrigen ist nur noch Reichweite offen, am weitesten bei Regel 7 — ihr Check misst auf Baseline `2026-07-28`, den Pagination-Verlust gibt es aber auch auf `2025-11-25`. Die Regeln 13 und 14 haben ihre Checks binnen eines Tages bekommen, nachdem sie hier geschrieben waren; das gehört dazugesagt, bevor jemand einer Zeile vertraut: Der Katalog bewegt sich schneller als dieser Skill. Was Lücke ist und was benannter Rand, steht je Zeile in `SKILL.md`.
