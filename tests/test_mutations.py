...
Der eigentliche Test dieser Suite. Alles andere hier stellt sicher, dass er
nicht schrumpft, ohne dass es jemand merkt.
"""
...
def test_mutation_is_caught(mutation: Mutation, fixture_repo: Path) -> None:
    ...
    assert mutation.expect in message, (
        f"Check {check.number} wurde rot, aber mit einem anderen Befund als "
...
def test_mutation_leaves_the_other_checks_alone(
    ...
    """Die mutierte Prüfung ist rot — aber nicht der halbe Baum.
    ...
    und belegt über die einzelne Prüfung nichts. Zugesichert wird deshalb, dass
    die Mutation zielt.

    Ausgenommen sind die Prüfungen, die per Konstruktion mithängen — wer
    `reference/` löscht, trifft alle vier Vorlagen-Prüfungen, und das ist
    korrekt, nicht ungenau.
    """
    check = BY_NUMBER[mutation.check]
    mutation.apply(fixture_repo)
    ...
    assert len(still_red) < len(others), (
        ...
        f"nicht auf Check {check.number}, sie zerstört den Baum. Eine solche "
        "Mutation belegt über die einzelne Prüfung nichts."
    )


def test_check_12_catches_what_13_and_14_cannot(fixture_repo: Path) -> None:
    """Die Daseinsberechtigung von Check 12, als Test.
    ...
    bewachen — und deshalb gibt es 12.

    Wird dieser Test rot, weil 13 oder 14 die Lage plötzlich selbst bemerken,
    ist das eine gute Nachricht mit Handlungsbedarf: Dann ist 12 überflüssig
    ...
    BY_NUMBER[13].run(fixture_repo)
    BY_NUMBER[14].run(fixture_repo)

    with pytest.raises(CheckFailed) as raised:
        BY_NUMBER[12].run(fixture_repo)
...
def test_probe_file_is_removed_even_when_the_gate_is_blind(
    ...
    """Check 12 legt eine fehlerhafte Datei ab. Sie darf nie liegenbleiben.

    Eine vergessene Sonde setzt die Prüfungen 2, 3, 13 und 14 des nächsten
    Laufs auf eine Datei an, die niemand geschrieben hat — der Befund zeigte
    dann auf ein Gespenst. In der Shell-Fassung hing das an einem `trap`; hier
    an `try/finally`, und dieser Test ist der Beleg dafür.
    """
    ruff_toml = fixture_repo / "ruff.toml"
    ruff_toml.write_text(
        ruff_toml.read_text(encoding="utf-8").replace(
