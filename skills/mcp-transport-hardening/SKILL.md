...
Daraus folgt aber eine Anforderung an seine **Position**. Beim ersten Lauf stand er in der Mitte des Jobs, brach ihn ab, und die vier Schritte dahinter liefen nicht — Badge-Sync, Ketten-Tabelle und Dateiliste standen im Log als «nicht dran» und nicht als «bestanden». Ein Guard, den nur ein Mensch von aussen befriedigen kann, verdeckt in dieser Position genau die Guards, die ein Commit befriedigt: Der rote Lauf sagt dann «Description falsch», und ob daneben noch etwas rot wäre, weiss niemand. Er gehört deshalb an das Ende der Kette. Das ist `OPS-005` auf die eigene Pipeline angewandt, und es ist derselbe Unterschied wie dort — «bestanden» gegen «nicht gelaufen», nur diesmal von einem anderen Guard verursacht statt von einem fehlenden.
...
| im Bau | **`mcp-transport-hardening`** | **Dieser Skill:** kommt er hoch, weist er richtig ab? Die Umsetzung im Einstiegspunkt und das Abweis-Verhalten |
...
Stand des Katalogs: [`mcp-audit`](https://github.com/malkreide/mcp-audit-skill) v2.3.0, 120 Checks in zwölf Kategorien auf **zwei Spec-Baselines**. Die Zuordnung ist durch Lesen der Check-Dateien belegt, nicht aus den Titeln geschlossen; die Lücken sind benannt, statt sie durch eine ungefähre Zuordnung zu verdecken:
...
**Beim Nachziehen auf v2.3.0 hat die andere Richtung gefeuert.** `OPS-010` («Gegenprobe als Abnahmekriterium») ist neu, und sein `pdf_ref` nennt als Grund die *Katalog-Lücke gegen Regel 6* — die Lücke also, die diese Tabelle benannt hat. Der Katalog hat sie geschlossen; die Zeile zu Regel 6 sagt seit diesem Durchgang nicht mehr «kein Check». Zwei Einschränkungen gehören dazu, sonst behauptet die Zeile mehr als sie deckt: `OPS-010` ist `advisory` — ein Befund hält heute kein Audit auf —, und es fordert die Gegenprobe für *jede zentrale Zusicherung*, nicht nur für Sicherheitskontrollen. Es ist damit breiter als Regel 6 und in der Härte schwächer.

Von den drei weiteren Neuzugängen bewegt keiner eine Zeile: `DRIFT-008` (ein Live-Test muss die Quelle erreichen), `FID-007` (eine Zahlenspalte ohne Zahlen) und `OPS-009` (Herkunft der Fixture) gehören zur Datentreue und zur Evidenz, nicht zum Transport. Und die Zeile zu Regel 2 ist trotz eines naheliegenden Kandidaten geblieben, wo sie war — die Begründung steht jetzt in ihr, weil sie beim Lesen zweimal knapp umgekippt wäre.
...
| 2 — `host` als Saat der Allow-List | **kein Check.** [`SEC-016`](https://github.com/malkreide/mcp-audit-skill/blob/main/checks/SEC-016.md) liegt daneben, adressiert aber den umgekehrten Fall: `0.0.0.0` als *unbeabsichtigten* Bind (NeighborJack). Regel 2 setzt ein gewolltes `0.0.0.0`-Deployment voraus und fragt, ob der Bind die App erreicht. Das Urteil ist beim Nachlesen fast gekippt: Kriterium 4 von `SEC-016` stellt wörtlich dieselbe Frage («der gesetzte Wert erreicht den Listener»). Es bleibt trotzdem stehen, weil die zugehörige **Messung** den Startlog des Listeners liest — und der zeigt im Fehlerfall dieser Regel `Uvicorn running on http://0.0.0.0:…`. Genau das ist die uvicorn-Falle: Der Listener bindet richtig, die App hält sich weiter für Loopback und antwortet 421. Ein Kriterium, dessen Nachweis im Fehlerfall grün ist, deckt die Regel nicht |
...
| 4 — eingehende Host-Allow-List | [`SEC-024`](https://github.com/malkreide/mcp-audit-skill/blob/main/checks/SEC-024.md). Die ausgehende Gegenrichtung führt der Katalog in zwei Checks: [`SEC-005`](https://github.com/malkreide/mcp-audit-skill/blob/main/checks/SEC-005.md) (DNS-Pinning gegen TOCTOU) und [`SEC-028`](https://github.com/malkreide/mcp-audit-skill/blob/main/checks/SEC-028.md) (der Guard sagt unterscheidbar, *warum* er abgewiesen hat). Beide gehören nicht hierher — sie stehen hier, damit der Zeiger vollständig ist |
| 5 — der Negativtest muss aus dem eigenen Grund rot werden | **teilweise.** [`DRIFT-003`](https://github.com/malkreide/mcp-audit-skill/blob/main/checks/DRIFT-003.md) — «Kein Test-Assert wird vom Degradationspfad erfüllt» — ist dieselbe Klasse: ein Test, der aus dem falschen Grund besteht. Die dort geführten Ausprägungen sind andere; der Transportfall — `evil.example.com` wird auch von einer Loopback-Fallback-Policy abgewiesen — steht nicht darin. Seit v2.2.0 trägt [`SEC-028`](https://github.com/malkreide/mcp-audit-skill/blob/main/checks/SEC-028.md) denselben Griff ein zweites Mal und nennt diese Regel dafür beim Namen — allerdings gebunden an `tools_make_external_requests == true` und angewandt auf den Egress-Guard. Zwei Fundstellen, derselbe fehlende Fall |
| 6 — der Mutationstest ist das Abnahmekriterium | [`OPS-010`](https://github.com/malkreide/mcp-audit-skill/blob/main/checks/OPS-010.md) — «Gegenprobe als Abnahmekriterium». Bis Katalog-v2.2.0 stand hier «kein Check»; `OPS-010` ist gegen genau diese Lücke geschrieben und nennt sie in seinem `pdf_ref`. Es verlangt die Gegenprobe für **jede zentrale Zusicherung**, nicht nur für Sicherheitskontrollen, und führt die nicht anschlagende Mutation ausdrücklich als Ergebnis statt als Ausschuss. Zwei Untervarianten hat es, die diese Regel nicht führt: die Fake-Uhr, unter der eine Wanduhr-Zusicherung nicht widerlegbar ist, und der globale Patch auf ein fremdes Modul. Einschränkung: `adoption: advisory` — der Befund fällt, das Audit hält er nicht auf |
| 7 — die Testharness als eigene Fehlerquelle | **kein Check**, aber ein benachbarter: [`OPS-005`](https://github.com/malkreide/mcp-audit-skill/blob/main/checks/OPS-005.md) — «Pipeline unterscheidet ‹bestanden› von ‹nicht gelaufen›» — teilt die Frage, wie viel ein grüner Lauf belegt. Dazu seit v2.3.0 `OPS-010`, dessen Kriterium «kein Test patcht ein fremdes Modul global» aus derselben Familie stammt — es benennt aber eine **andere** Falle als die (b) dieser Regel: dort wandert ein Patch prozessweit in fremde Tests, hier verdeckt ein Patch auf der falschen Ebene dauerhaft ein Attribut und hängt die Suite. Die vier Harness-Fälle dieser Regel stehen in keinem Check |
...
Wer nach den Regeln 1, 3, 4, 6 und 8–12 sowie 14 baut, besteht die dort genannten Checks. Für die Regeln 2 und 7 gilt das nicht: Sie beschreiben Fehler, die dieser Katalog derzeit nicht sieht. Bei den Regeln 5 und 13 liegt je ein Check daneben oder deckt eine Hälfte — `DRIFT-003` fängt bei Regel 5 die Klasse, aber nicht den Transportfall, und `OPS-005` bei Regel 13 den Guard, der nie gegen `main` lief, aber nicht den Zweig, der vor ihm abzweigte. Ein bestandenes Audit ist dort kein Beleg, dass der Nachweis trägt.

Regel 6 ist der Sonderfall in dieser Aufzählung und steht deshalb nicht kommentarlos bei den gedeckten: Sie ist gedeckt, aber `advisory`. Ein Server kann `OPS-010` verfehlen und das Audit trotzdem bestehen — die Regel ist im Katalog angekommen, ihre Härte noch nicht.
