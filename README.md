...
> Claude skill for systematic audits of MCP servers against a curated corpus of best-practice standards. **97 checks**, 12 categories, with a Swiss compliance layer for public administration and a data-fidelity layer for data-source servers.
...
Output: profile-driven selection of the ~30 applicable checks out of 97, automated verification of every `automated` / `config_check` / `documentation_check` mode, findings stubs for `code_review` / `runtime_test` modes, and a full audit report from the template — all under `<repo>/audits/YYYY-MM-DD-<server-name>/`.

## Standards provenance

The 97 checks come from two curated best-practice documents plus five layers of our own (Swiss compliance, data fidelity, identity, upstream drift, dependency resolution), in auditable form. Every check carries a `pdf_ref` reference to its source in the frontmatter.
...
| **Operational practice** | Test strategy, documentation standard and phase architecture from appendix C; plus audit honesty (`OPS-004`), pipeline honesty (`OPS-005`) and documented commands that actually run (`OPS-007`) — our own findings: a report that closed an unexplained remainder with a guess ([termdat-mcp#11](https://github.com/malkreide/termdat-mcp/issues/11)), a test suite no workflow ever ran ([mcp-continuous-auditor#29](https://github.com/malkreide/mcp-continuous-auditor/pull/29)), and a setup instruction that is a syntax error in PowerShell | 7 Checks (`OPS-*`) |
...
| `OPS` | Test strategy, documentation standard, phase architecture, audit honesty, pipeline honesty, reproducible verdicts, executable instructions | Appendix C + custom | 7 | — · 5 high · 2 medium |
...
The field is optional; when absent, `enforced` applies. Of 97 checks exactly three are `advisory`: `OPS-005`, `OPS-006` and `OPS-007`. `DEP-001`, `DRIFT-006`, `OBS-007` and `ARCH-014` took the same path and have since been promoted to `enforced` — the bridge is meant to carry a handful of new checks, not to fill up.
...
2. **Load the catalogue** — parse all 97 checks
...
- ✅ Reference summary
- ✅ Check catalogue: **97 checks, all 12 categories complete**
