...
The field is optional; when absent, `enforced` applies. Of 96 checks exactly two are `advisory`: `OPS-005` and `OPS-006`. `DEP-001`, `DRIFT-006`, `OBS-007` and `ARCH-014` took the same path and have since been promoted to `enforced` — the bridge is meant to carry a handful of new checks, not to fill up.
